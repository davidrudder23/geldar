...
public class MyGdxGame extends ApplicationAdapter implements ApplicationListener, GestureListener, InputProcessor {
	...
	private Point cameraLocationAgainstMap;
...
//		}
//		
		cameraLocationAgainstMap = recenterCamera();
		System.out.println("cameraLocationAgainstMap="+cameraLocationAgainstMap );
		camera.update();
	...
	private Point recenterCamera() {
		...
		Point point = new Point((int)camCenterX, (int)camCenterY);
	...
	public int getTileXFromScreenX(int screenX) {
		int tileX = cameraLocationAgainstMap.getX()+(int)(screenX/camera.viewportHeight);
		return tileX;
	}
	
	public int getTileYFromScreenY(int screenY) {
	    //
		int tileY = cameraLocationAgainstMap.getY()+(int)(screenY/camera.viewportHeight);
		return tileY;
	}
